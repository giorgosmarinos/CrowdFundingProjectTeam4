...
namespace CrowdFundingProjectTeam4.Services
{
    public class UserService : IUserService
    {
        ...
        public List<FundingPackage> ReadFundingPackages(int projectid)
        {
            List<FundingPackage> ans = _cruwdFundingDBContext.FundingPackage.Where(fp => fp.OwnedByProject.ProjectId == projectid).Include(p => p.OwnedByProject).ToList();
            return ans;
        }
    }
}